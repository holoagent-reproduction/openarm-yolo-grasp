...
bool MotorControl::controlMIT(Motor& motor, double kp, double kd, double q,
  ...
  if (!sendData(motor.SlaveID, data)) {
    std::cerr << "CAN send failed for motor 0x" << std::hex << motor.SlaveID
              << std::dec << std::endl;
    return false;
  }
  if (!recv()) {
    std::cerr << "CAN response timeout for motor 0x" << std::hex
              << motor.SlaveID << std::dec << std::endl;
    return false;
  }
  return true;
}
...
bool MotorControl::sendData(uint16_t motor_id,
                            const std::array<uint8_t, 8>& data) {
  return canbus_.send(motor_id, data);
}

bool MotorControl::recv(int timeout_ms) {
  uint16_t id = 0;
  uint8_t len = 0;
  std::array<uint8_t, 64> data = {};
  if (!canbus_.recv(id, len, data, timeout_ms)) {
    return false;
  }
...
void MotorControl::recv_set_param_data() {
  ...
  uint8_t len = 0;
  std::array<uint8_t, 64> data = {};
  if (!canbus_.recv(id, len, data, 100)) {
    std::cerr << "CAN response timeout while reading a parameter" << std::endl;
    return;
  }
