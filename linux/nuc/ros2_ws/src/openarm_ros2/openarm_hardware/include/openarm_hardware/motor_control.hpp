...
class MotorControl {
 public:
  ...
  bool controlMIT(Motor& motor, double kp, double kd, double q, double dq,
                  double tau);
  void controlMIT2(Motor& motor, double kp, double kd, double q, double dq,
                   double tau);
  bool sendData(uint16_t motor_id, const std::array<uint8_t, 8>& data);
  bool recv(int timeout_ms = 100);
